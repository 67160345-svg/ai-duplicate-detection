...
def load_image_from_url_or_path(image_source: str) -> np.ndarray:
  """โหลดรูปภาพจาก URL หรือ Local Path และแปลงเป็น RGB NumPy Array"""
  headers = {
      'User-Agent': (
  ...
  if img_bgr is None:
    raise ValueError(f'Cannot decode image: {image_source}')
...
def preprocess_image(
  ...
  """1. Resize 2. Denoise (Bilateral Filter เพื่อความเร็ว) 3. Normalization"""
  # 1. Resize
  resized_img = cv2.resize(img_rgb, target_size, interpolation=cv2.INTER_LINEAR)

  # 2. Denoise แบบ Fast (รักษาขอบภาพ)
  denoised_img = cv2.bilateralFilter(resized_img, d=5, sigmaColor=50, sigmaSpace=50)
...
def calculate_quality_scores(img_rgb: np.ndarray) -> Dict[str, float]:
  """คำนวณค่าคุณภาพภาพสำหรับใส่ลง Response Schema (สเกล 0.0 - 1.0)"""
  ...
  noise_score = float(np.clip(noise_sigma / 50.0, 0.0, 1.0))
